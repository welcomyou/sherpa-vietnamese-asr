...
def compute_emb_fbank(audio, sr=SAMPLE_RATE):
    ...
    scaled_audio = audio * 32768.0
    fb = knf.OnlineFbank(_knf_emb_opts)
    fb.accept_waveform(sr, scaled_audio.tolist())
    fb.input_finished()
...
class PureOrtDiarizer:
    ...
    def initialize(self):
        ...
        Z = compute_ort_threads(self.num_threads)
        print(f"[PureORT] Threads: cpu_threads={self.num_threads}, Z={Z} (seg={Z}, emb={Z})")

        opts_seg = ort.SessionOptions()
        opts_seg.intra_op_num_threads = Z
        ...
        opts_emb.intra_op_num_threads = Z
        opts_emb.inter_op_num_threads = 1
        ...
        self.seg_sess = ort.InferenceSession(
            os.path.join(self.onnx_dir, "segmentation-community-1.onnx"),
            opts_seg, providers=prov)
    ...
    def process(self, audio_file, progress_callback=None,
        ...
        # pyannote Inference.infer() applies Powerset(soft=False) → binary 0/1
    ...
    def _extract_embeddings(self, audio, binarized, clean_binarized,
        ...
        # --- 1. Single-stream fbank (8x faster than per-chunk) ---
        import kaldi_native_fbank as knf
        # Ensure fbank opts are initialized
        compute_emb_fbank(np.zeros(1600, dtype=np.float32), SAMPLE_RATE)
        padded_audio = np.pad(audio, (0, CHUNK_SAMPLES))
        scaled = padded_audio * 32768.0
        fb = knf.OnlineFbank(_knf_emb_opts)
        fb.accept_waveform(SAMPLE_RATE, scaled.tolist())
        fb.input_finished()
        ...
        # Extract per-chunk fbank windows + CMVN
        frame_shift_samples = int(SAMPLE_RATE * 0.01)  # 160
        all_fbanks = np.zeros((num_chunks, frames_per_chunk, 80), dtype=np.float32)
        for c in range(num_chunks):
            ...
            if n > 0:
                all_fbanks[c, :n] = all_raw_frames[fbank_start:fbank_end]
            all_fbanks[c] -= all_fbanks[c].mean(axis=0, keepdims=True)

        # --- 2. Batched encoder with IOBinding ---
        batch_size = 32
        all_frame_feats = [None] * num_chunks
        ...
        for b_start in range(0, num_chunks, batch_size):
            ...
            batch = all_fbanks[b_start:b_end]  # contiguous slice
            io_binding.bind_cpu_input("fbank_features", batch)
